...
void customerMenu()
{
    ...
    do
    {
        ...
        {
        case 1:
            viewStock(inventory, inventoryCount, "AVAILABLE STOCK");
            break;
        case 2:
...
void managerMenu()
{
    ...
    do
    {
        ...
        case 3:
            ...
            break;
        case 4:
            viewStock(inventory, inventoryCount, "CURRENT STOCK");
            break;
        case 5:
            manageSales();
            exportDataToJS();
...
void supervisorMenu()
{
    ...
    do
    {
        ...
        case 1:
            ...
            break;
        case 2:
            viewStock(inventory, inventoryCount, "STOCK");
            break;
        }
    } while (choice != 3);
}
...
Date getCurrentDate()
{
    ...
    Date d = {tm_info.tm_mday, tm_info.tm_mon + 1, tm_info.tm_year + 1900};
    return d;
}
...
int isExpired(Date expiry)
{
    ...
    if (cur.year > expiry.year)
        return 1;
    if (cur.year == expiry.year && cur.month > expiry.month)
        return 1;
    if (cur.year == expiry.year && cur.month == expiry.month && cur.day > expiry.day)
        return 1;
    return 0;
}
...
void addProduct(Product *inv, int *count)
{
    ...
    printf("ID: ");
    scanf_s("%d", &n.ID);
    if (findProductByID(inv, *count, n.ID) != -1)
    {
        printf("Exists!\n");
        return;
    }
    printf("Name: ");
    scanf_s("%s", n.name, (unsigned)_countof(n.name));
    printf("Price: ");
    scanf_s("%f", &n.price);
    printf("Qty: ");
    scanf_s("%d", &n.quantity);
    printf("Section: ");
    scanf_s("%s", n.section, (unsigned)_countof(n.section));
    printf("Expiry (DD MM YYYY): ");
    scanf_s("%d %d %d", &n.expiryDate.day, &n.expiryDate.month, &n.expiryDate.year);
    inv[*count] = n;
    (*count)++;
...
void viewStock(Product *inv, int count, const char *title)
{
    ...
    for (int i = 0; i < count; i++)
        printf("%-5d %-20s %-10.2f %-10d %-15s %02d/%02d/%d\n", inv[i].ID, inv[i].name, inv[i].price, inv[i].quantity, inv[i].section, inv[i].expiryDate.day, inv[i].expiryDate.month, inv[i].expiryDate.year);
}
...
void sellProducts(Product *inv, int invCount, Product *cart, int *cartCount)
{
    ...
    *cartCount = 0;
    do
    {
        ...
        if (choice == 1)
            viewStock(inv, invCount, "STOCK");
        else if (choice == 2)
            addToCart(inv, invCount, cart, cartCount);
        else if (choice == 3 && *cartCount > 0)
...
void addToCart(Product *inv, int invCount, Product *cart, int *cartCount)
{
    ...
    if (input[0] >= '0' && input[0] <= '9')
        idx = findProductByID(inv, invCount, atoi(input));
    if (idx == -1)
        idx = findProductByName(inv, invCount, input);
    if (idx == -1)
    {
        printf("Not found!\n");
        clearBuffer();
    ...
    if (qty > inv[idx].quantity)
    {
        printf("Shortage! (Available: %d)\n", inv[idx].quantity);
        return;
    }
    cart[*cartCount] = inv[idx];
...
void reviewAndCheckout(Product *inv, int invCount, Product *cart, int *cartCount)
{
    ...
    do
    {
        ...
        else if (choice == 2)
        {
            ...
            if (diff > 0 && diff > inv[iIdx].quantity)
            {
...
void viewAnalytics()
{
    ...
    {
        for (int i = 0; i < 5; i++)
            if (_stricmp(s.section, sections[i]) == 0)
                counts[i] += s.quantity;
    }
    fclose(f);
    printf("\n--- SECTION ANALYTICS ---\n");
...
void manageSales()
{
    ...
    {
        printf("PID: ");
        scanf_s("%d", &saleRules[ruleCount].productID);
        printf("Buy Qty: ");
        scanf_s("%d", &saleRules[ruleCount].buyQty);
        printf("Pay Qty: ");
        scanf_s("%d", &saleRules[ruleCount].payQty);
        ruleCount++;
        saveRules();
    }
    else
...
{
    FILE *f = NULL;
    fopen_s(&f, RULES_FILE, "r");
    if (f == NULL)
        return;
    ruleCount = 0;
    while (ruleCount < MAX_SALE_RULES && fscanf_s(f, "%d %d %d", &saleRules[ruleCount].productID, &saleRules[ruleCount].buyQty, &saleRules[ruleCount].payQty) == 3)
        ruleCount++;
    fclose(f);
}
...
{
    FILE *f = NULL;
    fopen_s(&f, PENDING_ORDER_FILE, "w");
    if (f == NULL)
        return;
    fprintf(f, "%d %.2f\n", count, total);
    for (int i = 0; i < count; i++)
...
{
    FILE *f = NULL;
    fopen_s(&f, PENDING_ORDER_FILE, "r");
    if (f == NULL)
    {
        printf("No pending orders to confirm.\n");
        return;
    }

    int count;
    float total;
    if (fscanf_s(f, "%d %f", &count, &total) != 2)
    {
        fclose(f);
        return;
    }
